...
def recommend(results_dir, model_dir, train_test_dir,
    ...
    train_data, test_data = load_train_test(
        train_test_dir, user_id_col, item_id_col)
    ...
    if user_id in eval_items:
        assume_interacted_items = eval_items[
            user_id]['assume_interacted_items']
        ...
        if recommended_items:
            for recommended_item in recommended_items:
                ...
                if metadata_fields is not None:
                    ...
                    record = train_data[train_data[
                        item_id_col] == recommended_item]
                    if not record.empty:
                        for field in metadata_fields:
                            print("\t {} : {}".format(
                                field, record[field].values[0]))

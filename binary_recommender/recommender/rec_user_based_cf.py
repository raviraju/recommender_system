...
class UserBasedCFRecommender(Recommender):
    ...
    def __get_similar_users(self, user_id):
        ...
        #print(user_id)
        if user_id in self.user_similarity_matrix_df.index:
            similar_users_weights = self.user_similarity_matrix_df[user_id]
            sorted_similar_users_weights = similar_users_weights.sort_values(ascending=False)
            #print(len(sorted_similar_users_weights))
            sorted_similar_users_weights = sorted_similar_users_weights.drop(user_id)#drop current user
            sorted_similar_users_weights = sorted_similar_users_weights[sorted_similar_users_weights > 0]#score > 0
            #print(len(sorted_similar_users_weights))
            #print(sorted_similar_users_weights)
            #input()            
        else:
            sorted_similar_users_weights = pd.Series()
        return sorted_similar_users_weights
    ...
    def __generate_top_recommendations(self, user_id, known_interacted_items):
        ...
        """
        # Calculate a weighted average of the scores in cooccurence matrix for all similar users.
        items_to_recommend = []        
